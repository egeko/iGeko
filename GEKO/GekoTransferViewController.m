...
@interface GekoTransferViewController ()

@end

@implementation GekoTransferViewController {
...
- (void)makeTheView {
    ...
    amountLabel.text = @"Montant à transférer: ";
    ...
    [transfer setTitle:@"Transférer" forState:UIControlStateNormal];
    ...
    if ([amount.text length] > 0 && [chosenUser.text length] > 0) {
        ...
        [api getDestWithUserId:chosenUser.text AndCompletion:^(NSString *results){
            if ([results isEqual:@"OK"]) {
                ...
                isTransfering = YES;
                UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"Transfert" message:[NSString stringWithFormat:@"Transferer %@€ à %@?", amount.text, userName] delegate:self cancelButtonTitle:@"Annuler" otherButtonTitles:@"OK", nil];
                [av show];
            } else {
                // server error
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 1) {
        if (isTransfering) {
            GekoAPI *api = [[GekoAPI alloc] init];
            [api transfertFromAccount:userId ToAccount:chosenUser.text WithAmount:amount.text AndCompletion:^(NSString *results){
                isTransfering = NO;
                [self.navigationController popViewControllerAnimated:YES];
            }];
        } else {
